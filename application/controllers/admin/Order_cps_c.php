<?php
class Order_cps_c extends CI_Controller
{
	...
	public function __construct()
	{
		...
		$this->page_redirect="admin/Order_cps_c";								
		$this->load->model("order_cps_m");
	}
	...
	{		
		...
		$data["tbl_hdr"]=array("CPS ID","Account ID","Account name","Photo","Status");		
		$row=$this->order_cps_m->index();		
		$i=0;
		if($row==TRUE)
		{
			foreach($row as $value):
			$data["tbl_body"][$i]=array(				
										"<a href=".base_url($this->page_redirect.'/detail/'.$value->cps_id)." title='CV Paid Search Detail'>".$value->cps_code."</a>",																																																																										
										$value->acc_code,
										$value->acc_name,
	...
	{
		$data["detail"]=$this->order_cps_m->index($id);		
		$data['cancel'] = $this->page_redirect;
		$this->load->view('template/header');
		$this->load->view('template/left');		
		$this->load->view('admin/order_cps_detail.php',$data);
		$this->load->view('template/footer');	
	}				
